...
router.get("/list", async (req: Request, res: Response) => {
  ...
  if (categoryList.length === 0)
    return res
      .status(NOT_FOUND)
      .send(genResponseForm(null, null, "No category in list"));

  // respone the category list
  console.log(categoryList);
  res.send(
    genResponseForm({ categoryList }, null, "get category list successful")
  );
});
  ...
  async (req: ServerRequest<AddedCategory>, res: Response) => {
    ...
    if (!category)
      return res
        .status(NOT_FOUND)
        .send(
          genResponseForm(null, null, "Cannot found the category with given id")
        );
...
router.post(
  ...
  async (req: RequestWithCategory<AddedCategory>, res: Response) => {
    ...
    if (error) {
      ...
      return res
        .status(BAD_REQUEST)
        .send(genResponseForm(null, errors, "Invalid params"));
    }
    ...
    // check duplicated
    const isExistedCategory = await cateRepo.getCategoryByName(
    ...
    if (isExistedCategory)
      return res
        .status(BAD_REQUEST)
        .send(
          genResponseForm(null, null, "The given category is already existed")
        );
...
router.put(
  ...
  async (req: RequestWithCategory<AddedCategory>, res: Response) => {
      ...
      return res
        ...
        .send(
          genResponseForm(
            null,
            null,
            "The category with the given id is not found"
          )
        );
    ...
    // update to db
    const result = await cateRepo.updateCategory(
      category,
      req.body.categoryName
    );
